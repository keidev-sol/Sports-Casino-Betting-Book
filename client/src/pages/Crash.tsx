...
import type { CrashSnapshot } from '@shared/types';

function CurveChart({ multiplier, state }: { multiplier: number; state: string }) {
  ...
  const color = crashed ? '#ff4d6a' : multiplier > 2 ? '#19c964' : '#e8b923';
...
export default function Crash() {
  ...
  const [snap, setSnap] = useState<CrashSnapshot>({ state: 'betting', multiplier: 1, players: [], history: [] });
  const [wager, setWager] = useState(50);
  const [autoCashout, setAutoCashout] = useState('');
  const [betting, setBetting] = useState(false);
  const hasBet = useRef<boolean>(false);

  useEffect(() => {
    const unsub = subscribe('crash', (f) => {
      if (f.type === 'state') {
        setSnap(f.data as CrashSnapshot);
        if ((f.data as CrashSnapshot).state === 'betting') hasBet.current = false;
      } else if (f.type === 'tick') {
        const d = f.data as { multiplier: number };
        setSnap((s) => ({ ...s, multiplier: d.multiplier }));
      } else if (f.type === 'cashout') {
        const d = f.data as { username: string; multiplier: number; payout: number };
        if (d.username === user?.username) {
          notify(`Cashed out at ${d.multiplier}× → +${d.payout} 🚀`, 'success');
          refreshUser();
        }
      }
    });
        ...
        <div className="panel space-y-4 p-4">
          <div>
            ...
            <div className="flex gap-2">
              <input
                type="number"
                value={wager}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setWager(Number(e.target.value))}
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. 2.0"
              value={autoCashout}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAutoCashout(e.target.value)}
              className="w-full rounded-xl border border-white/5 bg-ink-800 px-3 py-2.5 font-mono text-white outline-none focus:border-neon-purple/50"
            />
          </div>
