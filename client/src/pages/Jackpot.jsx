...
const SEGMENT_COLORS = ['#7c5cff', '#00e701', '#ff4d8d', '#ffb800', '#22d3ee', '#ff3b5c', '#9d7bff', '#3dff7a'];
...
export default function Jackpot() {
  ...
  const [snap, setSnap] = useState({ state: 'open', entries: [], pot: 0, history: [] });
  ...
  useEffect(() => {
    return subscribe('jackpot', (f) => {
      if (f.type === 'state') {
        setSnap((prev) => {
          if (f.data.winner && (!prev.winner || prev.winner.username !== f.data.winner.username)) {
            if (f.data.winner.username === user?.username) {
              notify(`🏆 You won the jackpot — ${f.data.winner.prize} credits!`, 'success');
            }
            refreshUser();
          }
          return f.data;
