...
import type { RouletteSnapshot } from '@shared/types';

const COLORS: Record<string, { bg: string; text: string; label: string; payout: string }> = {
...
export default function Roulette() {
  ...
  const [snap, setSnap] = useState<RouletteSnapshot>({ state: 'betting', bets: [], history: [], slots: [], result: null, endsAt: null });
  const [amount, setAmount] = useState(25);

  useEffect(() => {
    return subscribe('roulette', (f) => {
      if (f.type === 'state') setSnap(f.data as RouletteSnapshot);
      else if (f.type === 'win') {
        const data = f.data as { username: string; color: string; payout: number };
        if (data.username === user?.username) {
          notify(`Roulette win on ${data.color} → +${data.payout} 🎉`, 'success');
          refreshUser();
        }
      }
    });
  ...
  const totalOn = (color: string) => snap.bets.filter((b) => b.color === color).reduce((s, b) => s + b.amount, 0);
