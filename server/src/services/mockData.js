...
export function mockSports() {
  return SPORT_CATALOG;
}

export function mockOdds(sportKey, { eventsPerSport = 8 } = {}) {
  ...
  for (let i = 0; i < eventsPerSport; i++) {
    ...
    const margin = 1.06;
    const dec = (p) => round(Math.max(1.04, 1 / (p * margin)));
    ...
    const bookmakers = BOOKMAKERS.map((bm, bi) => {
      const wob = (o) => round(o.price * (1 + (bi - 1) * 0.015 + (rng() - 0.5) * 0.02));
      return {
        key: bm.key,
        title: bm.title,
