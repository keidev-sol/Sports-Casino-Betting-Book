...
import type { CrashSnapshot, CrashHistory } from '../../../shared/types.js';
...
export class CrashGame extends EventEmitter {
  clientSeed = 'crow-public-seed';
  nonce = 0;
  state: CrashSnapshot['state'] = 'betting';
  multiplier = 1.0;
  players = new Map<string, CrashEntry>();
  history: CrashHistory[] = [];
  round: CrashRound | null = null;
  ...
  private _beginRunning(): void {
    ...
    const loop = (): void => {
      ...
      if (this.round && this.multiplier >= this.round.bustAt) {
        return this._crash();
      }
  ...
  placeBet(userId: string, username: string, wager: number, autoCashout: number | null): CrashSnapshot {
  ...
  cashout(userId: string): { multiplier: number; payout: number; balance: number } {
    if (this.state !== 'running') throw new Error('Cannot cash out right now');
    return this._cashout(userId, this.multiplier);
  }

  private _cashout(userId: string, at: number): { multiplier: number; payout: number; balance: number } {
